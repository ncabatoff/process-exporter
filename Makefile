...
PREFIX                  ?= $(shell pwd)
BIN_DIR                 ?= $(shell pwd)
DOCKER_IMAGE_NAME       ?= opvizorpa/process-exporter
...
dockertest:
	docker run --rm -it -v `pwd`:/go/src/github.com/opvizor/process-exporter golang:1.10  make -C /go/src/github.com/opvizor/process-exporter test

dockerinteg:
	docker run --rm -it -v `pwd`:/go/src/github.com/opvizor/process-exporter golang:1.10  make -C /go/src/github.com/opvizor/process-exporter build integ
