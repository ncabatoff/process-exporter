...
func TestGrouperBasic(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			GroupByName{
				"g1": Group{Counts{1, 1, 1, 1, 1, 1, 0, 0}, States{Zombie: 1}, 1,
					Memory{6, 7, 0}, starttime, 100, 0.25, 4, nil},
				"g2": Group{Counts{2, 2, 2, 2, 2, 2, 0, 0}, States{Running: 1}, 1,
...
func TestGrouperProcJoin(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			GroupByName{
				"g1": Group{Counts{2, 2, 2, 2, 2, 2, 0, 0}, States{Running: 1, Sleeping: 1}, 2,
			...
			GroupByName{
				"g1": Group{Counts{4, 4, 4, 4, 4, 4, 0, 0}, States{Running: 2}, 2,
...
func TestGrouperNonDecreasing(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			GroupByName{
				"g1": Group{Counts{1, 1, 1, 1, 1, 1, 0, 0}, States{}, 1, Memory{1, 5, 0}, starttime, 4, 0.01, 2, nil},
			},
		}, {
			[]IDInfo{},
			GroupByName{
				"g1": Group{Counts{1, 1, 1, 1, 1, 1, 0, 0}, States{}, 0, Memory{}, time.Time{}, 0, 0, 0, nil},
			},
		},
	}
	...
	tests := []struct {
	...
	}{
		{
			piinfot(p, n, Counts{}, Memory{}, Filedesc{1, 1}, []Thread{
				{ThreadID(ID{p, 0}), "t1", Counts{1, 2, 3, 4, 5, 6, 0, 0}},
				{ThreadID(ID{p + 1, 0}), "t2", Counts{1, 1, 1, 1, 1, 1, 0, 0}},
			}),
			GroupByName{
			...
			piinfot(p, n, Counts{}, Memory{}, Filedesc{1, 1}, []Thread{
				{ThreadID(ID{p, 0}), "t1", Counts{2, 3, 4, 5, 6, 7, 0, 0}},
				{ThreadID(ID{p + 1, 0}), "t2", Counts{2, 2, 2, 2, 2, 2, 0, 0}},
				{ThreadID(ID{p + 2, 0}), "t2", Counts{1, 1, 1, 1, 1, 1, 0, 0}},
			}),
			GroupByName{
			...
			piinfot(p, n, Counts{}, Memory{}, Filedesc{1, 1}, []Thread{
				{ThreadID(ID{p + 1, 0}), "t2", Counts{4, 4, 4, 4, 4, 4, 0, 0}},
				{ThreadID(ID{p + 2, 0}), "t2", Counts{2, 3, 4, 5, 6, 7, 0, 0}},
			}),
			GroupByName{
				"g1": Group{Counts{}, States{}, 1, Memory{}, tm, 1, 1, 2, []Threads{
