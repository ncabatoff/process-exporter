...
func (s MySuite) TestGrouperBasic(c *C) {
	...
	p1 := newProc(1, "g1", ProcMetrics{1, 1.5, 2, 3, 4, 5, 4, 400, 6, 7})
	p2 := newProc(2, "g2", ProcMetrics{2, 2.5, 3, 4, 5, 6, 40, 400, 7, 8})
	...
	got1 := gr.groups()
	want1 := GroupCountMap{
		"g1": GroupCounts{Counts{0, 0, 0, 0, 0, 0}, 1, 4, 5, time.Time{}, 4, 0.01},
		"g2": GroupCounts{Counts{0, 0, 0, 0, 0, 0}, 1, 5, 6, time.Time{}, 40, 0.1},
	}
	...
	p1.ProcMetrics = ProcMetrics{2, 2.5, 3, 4, 5, 6, 4, 400, 7, 8}
	p2.ProcMetrics = ProcMetrics{4, 4.5, 5, 6, 7, 8, 40, 400, 9, 10}
	...
	got2 := gr.groups()
	want2 := GroupCountMap{
		"g1": GroupCounts{Counts{1, 1, 1, 1, 1, 1}, 1, 5, 6, time.Time{}, 4, 0.01},
		"g2": GroupCounts{Counts{2, 2, 2, 2, 2, 2}, 1, 7, 8, time.Time{}, 40, 0.1},
	}
	...
	// though.
	p4 := newProc(4, "g2", ProcMetrics{1, 1, 1, 1, 1, 1, 80, 400, 1, 1})
	p2.ProcMetrics = ProcMetrics{5, 5.5, 6, 7, 8, 9, 40, 400, 10, 11}
	...
	got3 := gr.groups()
	want3 := GroupCountMap{
		"g1": GroupCounts{Counts{0, 0, 0, 0, 0, 0}, 1, 5, 6, time.Time{}, 4, 0.01},
		"g2": GroupCounts{Counts{1, 1, 1, 1, 1, 1}, 2, 9, 10, time.Time{}, 120, 0.2},
	}
	c.Check(got3, DeepEquals, want3, Commentf("diff %s", pretty.Compare(got3, want3)))

	p4.ProcMetrics = ProcMetrics{2, 2, 2, 2, 2, 2, 100, 400, 2, 2}
	p2.ProcMetrics = ProcMetrics{6, 6.5, 7, 8, 8, 9, 40, 400, 11, 12}
	...
	want4 := GroupCountMap{
		"g1": GroupCounts{Counts{0, 0, 0, 0, 0, 0}, 1, 5, 6, time.Time{}, 4, 0.01},
		"g2": GroupCounts{Counts{2, 2, 2, 2, 2, 2}, 2, 10, 11, time.Time{}, 140, 0.25},
	}
...
func (s MySuite) TestGrouperGroup(c *C) {
	...
	p1 := newProc(1, "g1", ProcMetrics{1, 1.5, 2, 3, 4, 5, 8, 400, 9, 10})
	_, err := gr.Update(procInfoIter(p1))
	c.Assert(err, IsNil)
	got1 := gr.Groups()
	want1 := GroupCountMap{
		"g1": GroupCounts{Counts{0, 0, 0, 0, 0, 0}, 1, 4, 5, time.Time{}, 8, 0.02},
	}
	...
	p1.ProcMetrics = ProcMetrics{2, 2.5, 3, 4, 5, 6, 12, 400, 10, 11}
	_, err = gr.Update(procInfoIter(p1))
	c.Assert(err, IsNil)
	got2 := gr.Groups()
	want2 := GroupCountMap{
		"g1": GroupCounts{Counts{1, 1, 1, 1, 1, 1}, 1, 5, 6, time.Time{}, 12, 0.03},
	}
...
func (s MySuite) TestGrouperNonDecreasing(c *C) {
	...
	p1.ProcMetrics = ProcMetrics{2, 2.5, 3, 4, 5, 6, 8, 400, 7, 8}
	p2.ProcMetrics = ProcMetrics{4, 4.5, 5, 6, 7, 8, 80, 400, 9, 10}
	p3 := newProc(3, "g2", ProcMetrics{1, 1, 1, 1, 1, 1, 8, 400, 1, 1})
	_, err = gr.Update(procInfoIter(p1, p2, p3))
	c.Assert(err, IsNil)

	got2 := gr.Groups()
	want2 := GroupCountMap{
		"g1": GroupCounts{Counts{1, 1, 1, 1, 1, 1}, 1, 5, 6, time.Time{}, 8, 0.02},
		"g2": GroupCounts{Counts{2, 2, 2, 2, 2, 2}, 2, 8, 9, time.Time{}, 88, 0.2},
	}
	c.Check(got2, DeepEquals, want2, Commentf("diff %s", pretty.Compare(got2, want2)))

	// Now update p3's metrics and kill p2.
	p3.ProcMetrics = ProcMetrics{2, 2.5, 3, 4, 5, 6, 8, 400, 7, 8}
	...
	want3 := GroupCountMap{
		"g1": GroupCounts{Counts{1, 1, 1, 1, 1, 1}, 1, 5, 6, time.Time{}, 8, 0.02},
		"g2": GroupCounts{Counts{3, 3.5, 4, 5, 8, 9}, 1, 5, 6, time.Time{}, 8, 0.02},
	}
	c.Check(got3, DeepEquals, want3, Commentf("diff %s", pretty.Compare(got3, want3)))

	// Now update p3's metrics and kill p1.
	p3.ProcMetrics = ProcMetrics{4, 4, 4, 4, 2, 1, 4, 400, 4, 4}
	_, err = gr.Update(procInfoIter(p3))
	c.Assert(err, IsNil)

	got4 := gr.Groups()
	want4 := GroupCountMap{
		"g1": GroupCounts{Counts{1, 1, 1, 1, 1, 1}, 0, 0, 0, time.Time{}, 0, 0},
		"g2": GroupCounts{Counts{5, 5, 5, 5, 5, 5}, 1, 2, 1, time.Time{}, 4, 0.01},
	}
